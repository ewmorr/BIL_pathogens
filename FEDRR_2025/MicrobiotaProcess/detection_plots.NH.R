...
taxon_detect_list.State_Site_Lure = taxon_detect(mpse=mpse2, groupVar = c("State", "Site", "date_m.dd"), abdVar = Abundance, dbPath = dbPath, speciesSearch = spp_search_terms)
...
state_site_detections.high_priority <- taxon_detect_list.State_Site$detections_tab %>%
    filter(Species %in% high_priority_spp & State == "NH")
...
state_site_detections.med_priority <- taxon_detect_list.State_Site$detections_tab %>%
    filter(Species %in% med_priority_spp & State == "NH")
...
state_site_detections.low_priority <- taxon_detect_list.State_Site$detections_tab %>%
    filter(Species %in% low_priority_spp & State == "NH")
...
p3
pdf("figures/FEDRR_2025_11182025/MicrobiotaProcess/NH_detections_lure_site.pdf", width = 10)
p1
p2
p3
dev.off()
