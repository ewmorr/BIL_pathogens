...
#asv_tab formatting
asv_tab.samps_bl = read.csv("data/FEDRR_2025_11182025/asv_tab.samps_and_baseline.csv", row.names = 1)

asvs_tax = read.table("data/FEDRR_2025_11182025/ASVs_taxonomy.tsv", header = T)
tax_bs = read.table("data/FEDRR_2025_11182025/ASVs_taxonomy_bootstrapVals.tsv", header = T)
head(tax_bs < 90)
asvs_tax[tax_bs < 80] = NA

asv_tab.samps_bl = t(asv_tab.samps_bl) %>% data.frame
asv_tab.samps_bl$ASV = rownames(asv_tab.samps_bl)
asv_tab.samps_bl %>% mutate(across(everything(), ~replace_na(., 0))) %>% data.frame
asv_tab.samps_bl = asv_tab.samps_bl %>% mutate(across(everything(), ~replace_na(., 0))) %>% data.frame
...
metadata$date = metadata$dateOrBaselineType
metadata$date[grep("^BL.", metadata$date)] = NA
metadata$date[grep("^neg.*", metadata$date)] = NA
...
metadata %>% head()
metadata$State.Site = paste(metadata$State, metadata$Site, sep = ".")
metadata %>%
    select(State.Site, mdy) %>%
    unique() %>%
    filter(!is.na(mdy)) %>%
    arrange(State.Site, mdy) %>%
    group_by(State.Site) -> site_date_df
site_date_df$samplePeriod = vector(mode = "numeric", length = nrow(site_date_df))

sites_uniq = site_date_df$State.Site %>% unique()

for(i in 1:length(sites_uniq)){
    n_dates = nrow(site_date_df[site_date_df$State.Site == sites_uniq[i],])
    site_date_df[site_date_df$State.Site == sites_uniq[i],"samplePeriod"] = 1:n_dates
}

metadata.period = left_join(metadata, site_date_df)
metadata.period %>% filter(BaselineType != "sample")
metadata.period[metadata.period$BaselineType == "BLF", "samplePeriod"] = "BL"
...
ny_search_terms.long = left_join(
    ...
    metadata.period %>% 
        select(SequenceID, State, Site, date, Lure, BaselineType, samplePeriod),
    by = "SequenceID"
) 
ny_search_terms.long$samplePeriod %>% unique()
...
#ny_search_terms.long[ny_search_terms.long$BaselineType == 'BLO', "samplePeriod"] = "BL"
ny_search_terms.long[ny_search_terms.long$BaselineType == 'BLF', "date"] = "BLF"
ny_search_terms.long[ny_search_terms.long$BaselineType == 'BLF', "samplePeriod"] = "BLF"

ny_search_terms.long %>% head()
ny_search_terms.long %>% filter(State == "NH") %>% print(n = Inf)

#uniq_dates = ny_search_terms.long$date %>% unique() %>% str_sort(., numeric = T)
ny_search_terms.long$State %>% unique()
# "MD" "OH" "VT" "NH" "WV" "ME"
# 

ny_search_terms.long %>% filter(State == "MD") %>% select(samplePeriod) %>% unique()

p1 = ggplot(ny_search_terms.long %>% filter(State == "MD"), 
       aes(x = factor(samplePeriod, levels = c("BL", 1,2,3,4,5,6, 7)), y = Lure, fill = factor(gen_sp, levels = c("species", "genus", "absent")) )
) +
    geom_tile() + 
    scale_fill_manual(
...
pdf("figures/FEDRR_2025_11182025/MD_spp_concern.pdf", width = 34, height = 11)
p1
dev.off()

ny_search_terms.long %>% filter(State == "OH") %>% select(samplePeriod) %>% unique()
p2 = ggplot(ny_search_terms.long %>% filter(State == "OH"), 
       aes(x = factor(samplePeriod, levels = c("BL", 1,2)), y = Lure, fill = factor(gen_sp, levels = c("species", "genus", "absent")) )
) +
    geom_tile() + 
    scale_fill_manual(
        values = c("red", "yellow", "grey"), 
...
pdf("figures/FEDRR_2025_11182025/OH_spp_concern.pdf", width = 34, height = 11)
p2
dev.off()

ny_search_terms.long %>% filter(State == "VT") %>% select(samplePeriod) %>% unique()
p3 = ggplot(ny_search_terms.long %>% filter(State == "VT"), 
...
pdf("figures/FEDRR_2025_11182025/VT_spp_concern.pdf", width = 34, height = 11)
p3
dev.off()

ny_search_terms.long %>% filter(State == "NH") %>% select(samplePeriod) %>% unique()
p4 = ggplot(ny_search_terms.long %>% filter(State == "NH"), 
       aes(x = factor(samplePeriod, levels = c("BLF", 1,2,3,4,5)), y = Lure, fill = factor(gen_sp, levels = c("species", "genus", "absent")) )
) +
    geom_tile() + 
    scale_fill_manual(
        values = c("red", "yellow", "grey"), 
...
pdf("figures/FEDRR_2025_11182025/NH_spp_concern.pdf", width = 34, height = 11)
p4
dev.off()


ny_search_terms.long %>% filter(State == "WV") %>% select(samplePeriod) %>% unique()
p5 = ggplot(ny_search_terms.long %>% filter(State == "WV"), 
       aes(x = factor(samplePeriod, levels = c(1,2,3,4,5,6)), y = Lure, fill = factor(gen_sp, levels = c("species", "genus", "absent")) )
) +
    geom_tile() + 
    scale_fill_manual(
        values = c("red", "yellow", "grey"), 
...
pdf("figures/FEDRR_2025_11182025/WV_spp_concern.pdf", width = 34, height = 11)
p5
dev.off()

ny_search_terms.long %>% filter(State == "ME") %>% select(samplePeriod) %>% unique()
p6 = ggplot(ny_search_terms.long %>% filter(State == "ME"), 
       aes(x = factor(samplePeriod, levels = c(1,2,3)), y = Lure, fill = factor(gen_sp, levels = c("species", "genus", "absent")) )
) +
    geom_tile() + 
    scale_fill_manual(
        values = c("red", "yellow", "grey"), 
...
pdf("figures/FEDRR_2025_11182025/ME_spp_concern.pdf", width = 34, height = 11)
p6
dev.off()
