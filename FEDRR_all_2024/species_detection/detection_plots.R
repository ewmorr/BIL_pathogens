...
otus <- as.matrix(read.csv("data/FEDRR_all_2024/ASVs_counts.tsv", row.names = 1, sep = "\t"))
tax <- as.matrix(read.csv("data/FEDRR_all_2024/ASVs_taxonomy.tsv", row.names = 1, sep = "\t"))
sam <- as.data.frame(read.csv("data/2024_metadata/collated/collated_metadata_NY-OH-IA-NH_02102025.redo.csv", row.names = 1))
colnames(otus) = sub("_.*", "", colnames(otus))

sam$dateOrBaselineType %>% unique()

sam$date = paste0(sam$dateOrBaselineType, "-2024")
sam$date[grep("^BL.", sam$date)] = NA
sam$date[grep("^neg.", sam$date)] = NA
sam$date %>% unique()
sam$mdy = lubridate::mdy(sam$date)
sam$mdy %>% unique()
...
mpse2
mpse2 <- mpse2 %>% dplyr::filter(State == "IA")

mpse2.noZeros = mpse2 %>% dplyr::filter(Abundance > 0)
mpse2.noZeros
write.csv(mpse2.noZeros, "data/FEDRR_all_2024/IA_spp_list.csv", row.names = F, quote = F)
...
taxon_detect_list.State_Site_Lure$db_matches %>% 
...
p3
pdf("figures/FEDRR_all_2024/detections_IA.pdf", width = 10)
p1
p2
p3
dev.off()

write.csv(taxon_detect_list.State_Site_Lure$detections_tab, "data/FEDRR_all_2024/IA_spp_of_concern_detections.csv", row.names = F, quote = F)


# three group vars
state_site_lure_detections.high_priority %>%
ggplot(aes(x = Site, y = Lure, fill = Detections)) +
