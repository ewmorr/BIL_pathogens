...
metadata.period[metadata.period$BaselineType == "BLO", "samplePeriod"] = "BL"
...
p1 = ggplot(ny_search_terms.long %>% filter(State == "NY"), 
...
p3 = ggplot(ny_search_terms.long %>% filter(State == "IA"), 
...
p4 = ggplot(ny_search_terms.long %>% filter(State == "NH"), 
       aes(x = factor(samplePeriod, levels = c("BL", "BLF", 1,2,3,4,5,6)), y = Lure, fill = factor(gen_sp, levels = c("species", "genus", "absent")) )
) +
    geom_tile() + 
    scale_fill_manual(
        values = c("red", "yellow", "grey"), 
...
pdf("figures/FEDRR_all_2024/NH_spp_concern.pdf", width = 34, height = 11)
p4
dev.off()
